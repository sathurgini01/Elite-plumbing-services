...
export function AreasCoveredView() {
  ...
  return (
    <div id="coverage-view" className="font-sans text-[#E0E0E0] bg-[#0A0A0B] py-16 px-4">
      <div className="max-w-6xl mx-auto">
        
        {/* Coverage Header info */}
        <div className="text-center max-w-2xl mx-auto mb-12">
          <span className="text-[10px] text-[#C5A059] font-mono uppercase tracking-[0.25em] block mb-2">Territory Coverage</span>
        ...
        {/* SEARCH WIDGET PANEL */}
        <div className="bg-[#0E0E10] rounded-sm border border-white/10 p-6 md:p-8 shadow-sm max-w-2xl mx-auto mb-16">
          <h3 className="text-center font-mono text-xs uppercase tracking-widest mb-6 text-[#C5A059]">
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 rounded-sm border border-white/10 bg-[#050505] px-4 py-3 text-xs focus:border-[#C5A059] focus:outline-none focus:ring-1 focus:ring-[#C5A059] uppercase tracking-widest font-mono text-[#E0E0E0]"
              required
            />
            <button
              type="submit"
              className="bg-[#C5A059] hover:bg-[#b08e4d] text-black font-bold uppercase text-xs tracking-widest px-6 py-3.5 rounded-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer"
            >
          ...
          {searched && (
            <div className="mt-6 border-t border-white/10 pt-6 animate-in fade-in duration-200">
              {result ? (
                ...
                <div className="bg-[#0A0A0B] border border-[#C5A059]/30 rounded-sm p-5 text-white">
                  <div className="flex items-start gap-3.5 text-xs sm:text-sm">
                    <CheckCircle2 className="h-6 w-6 text-[#C5A059] shrink-0 mt-0.5" />
                    <div className="space-y-2">
                      ...
                      <p className="mt-1 leading-relaxed text-white/60 text-xs font-light">
                        Postcode Prefix <strong className="font-bold text-[#C5A059] font-mono">{result.code}</strong> ({result.area}) is within our fast dispatch zone.
                      </p>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 text-[10px] uppercase font-mono">
                        <button
                          ...
                          className="bg-[#C5A059] hover:bg-[#b08e4d] text-black font-bold px-4 py-2 rounded-sm"
                        >
                ...
                <div className="bg-[#0A0A0B] border border-white/10 rounded-sm p-5 text-white">
                  <div className="flex items-start gap-3.5 text-xs sm:text-sm">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="mt-1 leading-relaxed text-white/50 text-xs font-light">
                        Postcode "<strong className="text-[#C5A059] font-mono">{query.toUpperCase()}</strong>" is outside our high-priority list. We can sometimes schedule gas audits or commercial boiler work here, but we cannot promise sub-60 response times.
                      </p>
                      ...
                      <div className="mt-4">
                        <a
                          href={BUSINESS_INFO.phoneHref}
                          className="bg-transparent hover:bg-white/5 text-[#C5A059] border border-[#C5A059]/20 font-bold px-5 py-2 rounded-sm inline-flex items-center gap-1.5 text-[10px] uppercase font-mono"
                        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-[#C5A059]/10 text-[#C5A059] p-3 rounded-sm inline-block">
            ...
            <div className="bg-[#0E0E10] p-4 rounded-sm border border-white/5 text-[11px] font-mono text-white/60 space-y-2">
          ...
          <div className="lg:col-span-8 bg-[#0E0E10] rounded-sm border border-white/10 p-6 md:p-8 shadow-xs grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-h-[480px] overflow-y-auto scrollbar-thin">
            {POSTCODE_AREAS.map((item) => (
              <div 
               ...
               >
                <div>
                  <span className="font-mono text-xs font-bold text-[#C5A059] block">{item.code}</span>
                  <span className="text-[10px] text-white/40 font-mono tracking-tight block truncate max-w-[120px] uppercase">{item.area}</span>
                </div>
                <span className="text-[9px] bg-[#C5A059]/10 text-[#C5A059] font-mono px-2 py-0.5 rounded-sm uppercase tracking-wider">
                  Active
                </span>
              </div>
            ))}
